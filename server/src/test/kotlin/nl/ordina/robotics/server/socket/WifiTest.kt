...
import nl.ordina.robotics.server.transport.cli.commands.parseWifiInfo
import nl.ordina.robotics.server.transport.cli.commands.parseWifiNetworks
...
class WifiTest {
    ...
    fun testConnectionInfoParsing() {
        ...
        Assertions.assertEquals("KPN79F826.", info.ssid)
        Assertions.assertEquals("-58 dBm", info.signal)
        Assertions.assertEquals("433.3 Mb/s", info.rate)
    }
    ...
    fun testWifiNetworksOutputParsing() {
        ...
        Assertions.assertEquals(rows, info.networks)
    }
}
